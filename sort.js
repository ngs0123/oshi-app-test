...
const sortDefs={
  ...
  debut:{label:'デビュー順',dirs:[['asc','古い方から'],['desc','新しい方から']]},
  birthYear:{label:'生まれ年順',dirs:[['desc','若い方から'],['asc','年上から']]},
  birthMonth:{label:'誕生月順',dirs:[['asc','1月から'],['desc','12月から']]},
  oshiStart:{label:'推し始め順',dirs:[['asc','古い方から'],['desc','最近から']]}
};
function sortValue(o,key){
  if(key==='join') return o.joinDate?Date.parse(o.joinDate+'T00:00:00'):null;
  if(key==='debut') return o.debutDate?Date.parse(o.debutDate+'T00:00:00'):null;
  if(key==='oshiStart') return o.startDate?Date.parse(o.startDate+'T00:00:00'):null;
...
function sortLabel(){const key=appSettings.listSortKey||'';return key&&sortDefs[key]?`並べ替え：${sortDefs[key].label}`:'並べ替え'}
...
function renderSortSheet(){
  ...
  body.innerHTML=`<div class="sortChoices">${Object.entries(sortDefs).map(([k,d])=>`<button type="button" data-sort="${k}" class="${key===k?'selected':''}">${d.label}</button>`).join('')}</div>${key&&sortDefs[key]?`<div class="sortDirTitle">向き</div><div class="sortChoices">${sortDefs[key].dirs.map(([v,l])=>`<button type="button" data-dir="${v}" class="${(appSettings.listSortDir||sortDefs[key].dirs[0][0])===v?'selected':''}">${l}</button>`).join('')}</div>`:''}`;
  body.querySelectorAll('[data-sort]').forEach(b=>b.onclick=()=>{const k=b.dataset.sort;appSettings.listSortKey=k;appSettings.listSortDir=sortDefs[k].dirs[0][0];saveSettings();renderSortSheet();renderList()});
...
// 入所日 / 活動開始日・デビュー日を編集フォームへ追加
const birthdayField=document.getElementById('birthdayMode')?.closest('.field');
if(birthdayField&&!document.getElementById('joinDate')){const f=document.createElement('div');f.className='field';f.innerHTML='<label>入所日 / 活動開始日 <span class="optional">任意</span></label><input id="joinDate" type="date"><div class="sub" style="margin-top:6px">事務所への入所日・活動開始日など</div>';birthdayField.before(f)}
const joinField=document.getElementById('joinDate')?.closest('.field');
if(joinField&&!document.getElementById('debutDate')){const f=document.createElement('div');f.className='field';f.innerHTML='<label>デビュー日 <span class="optional">任意</span></label><input id="debutDate" type="date"><div class="sub" style="margin-top:6px">未デビューの場合は空欄でOK</div>';joinField.after(f)}
const oldBlank=window.blankForm;if(oldBlank)window.blankForm=function(){oldBlank();if(document.getElementById('joinDate'))document.getElementById('joinDate').value='';if(document.getElementById('debutDate'))document.getElementById('debutDate').value=''};
const oldFill=window.fillForm;if(oldFill)window.fillForm=function(o){oldFill(o);if(document.getElementById('joinDate'))document.getElementById('joinDate').value=o.joinDate||'';if(document.getElementById('debutDate'))document.getElementById('debutDate').value=o.debutDate||''};
const oldGet=window.getFormData;if(oldGet)window.getFormData=function(id){const o=oldGet(id);o.joinDate=document.getElementById('joinDate')?.value||'';o.debutDate=document.getElementById('debutDate')?.value||'';return o};
const oldOpenDetail=window.openDetail;if(oldOpenDetail)window.openDetail=function(id){oldOpenDetail(id);const o=oshis.find(x=>x.id===id);const card=document.querySelector('#detailContent .detailInfoCard');if(!card)return;const add=(key,label,value)=>{if(!value||card.querySelector(`[data-${key}-info]`))return;const el=document.createElement('div');el.className='info';el.dataset[key+'Info']='1';const [y,m,d]=value.split('-').map(Number);el.innerHTML=`<b>${label}</b>${y}年${m}月${d}日`;card.appendChild(el)};add('join','入所日 / 活動開始日',o?.joinDate);add('debut','デビュー日',o?.debutDate)};
